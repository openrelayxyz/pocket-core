...
var (
	ReceiptKey = []byte{0x01} // key for the verified and stored EvidenceEncodable
	ClaimKey   = []byte{0x02} // key for pending claims
)

// "KeyForClaim" - Generates the key for the claim object for the state store
func KeyForClaim(_ sdk.Ctx, addr sdk.Address, header SessionHeader, evidenceType EvidenceType) ([]byte, error) {
	// validat the header
	...
	// validate the EvidenceEncodable type
...
// "KeyForEvidence" - Generates the key for EvidenceEncodable
