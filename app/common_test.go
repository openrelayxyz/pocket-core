...
func inMemTendermintNode(genesisState []byte, protoCodec bool) (*node.Node, keys.Keybase) {
	...
	// 	m := map[string]sdk.HostedBlockchain{"0001": {
	// 		ID:  PlaceholderHash,
	// 		URL: PlaceholderURL,
	// 	}}
	// 	p := NewPocketCoreApp(GenState, getInMemoryKeybase(), getInMemoryTMClient(), &sdk.HostedBlockchains{M: m}, logger, db, bam.SetPruning(store.PruneNothing))
	// 	return p
	// }
	...
	app := GetApp(protoCodec, c.Logger, db, traceWriter)
