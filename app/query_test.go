...
func TestQueryTx(t *testing.T) {
	...
	tx, err = nodes.Send(memProto, memCli, kb, cb.GetAddress(), kp.GetAddress(), "test", sdk.NewInt(1000))
	assert.Nil(t, err)
	assert.NotNil(t, tx)
...
func TestQueryDaoBalance(t *testing.T) {
	...
	<-evtChan // Wait for block
	got, err := gov.QueryDAO(memProto, memCli, 0)
	assert.Nil(t, err)
...
func TestQueryACL(t *testing.T) {
	...
	memCli, stopCli, evtChan := subscribeTo(t, tmTypes.EventNewBlock)
	<-evtChan // Wait for block
	got, err := gov.QueryACL(memProto, memCli, 0)
	assert.Nil(t, err)
...
func TestQueryDaoOwner(t *testing.T) {
	...
	memCli, stopCli, evtChan := subscribeTo(t, tmTypes.EventNewBlock)
	<-evtChan // Wait for block
	got, err := gov.QueryDAOOwner(memProto, memCli, 0)
	assert.Nil(t, err)
...
func TestQueryUpgrade(t *testing.T) {
	...
	<-evtChan // Wait for block
	var err error
	got, err := gov.QueryUpgrade(memProto, memCli, 0)
	assert.Nil(t, err)
