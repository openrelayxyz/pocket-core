	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				_ = unmarshalledDec.UnmarshalJSON(got)
...
func TestSerializationGocodecBinary(t *testing.T) {
	...
	bz, err := legacyCdc.MarshalBinaryLengthPrefixed(d)
	require.NoError(t, err)

	var d2 Dec
	err = legacyCdc.UnmarshalBinaryLengthPrefixed(bz, &d2)
	require.NoError(t, err)
	require.True(t, d.Equal(d2), "original: %v, unmarshalled: %v", d, d2)
...
func TestEmbeddedStructSerializationGocodec(t *testing.T) {
	...
	bz, err := legacyCdc.MarshalBinaryLengthPrefixed(obj)
	require.Nil(t, err)

	var obj2 testDEmbedStruct
	err = legacyCdc.UnmarshalBinaryLengthPrefixed(bz, &obj2)
	require.Nil(t, err)
